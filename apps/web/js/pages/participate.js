...
import { getPublishedForms, getPublicFormBundle, getOptionCounts, submitForm, isDemoMode, getModeBannerText } from '../services/public-api.js';
...
function formClosed(bundle) {
  ...
  if (form.global_deadline_at && new Date(form.global_deadline_at).getTime() < Date.now()) return { closed: true, reason: '전체 마감 시간이 지났어.' };
  if (form.max_responses !== null && form.max_responses !== undefined && Number(form.response_count) >= Number(form.max_responses)) return { closed: true, reason: '전체 제출 정원이 모두 찼어.' };
...
function optionClosed(questionId, option) {
  ...
  if (option.capacity !== null && option.capacity !== undefined && Number(getOptionCount(questionId, option.id)) >= Number(option.capacity)) return { closed: true, reason: '정원 마감' };
  return { closed: false, reason: '' };
}
...
function renderFormList() {
  formListNode.innerHTML = forms.length ? forms.map((form) => `
    <div class="form-item ${selectedBundle?.form?.id === form.id ? 'active' : ''}" data-form-id="${form.id}">
      ...
      ${form.global_deadline_at ? `<div class="subtle" style="margin-top:6px">전체 마감 ${escapeHtml(fmtDate(form.global_deadline_at))}</div>` : '<div class="subtle" style="margin-top:6px">상시 접수</div>'}
    </div>
  `).join('') : renderEmpty('공개된 신청 폼이 없어');
  formListNode.querySelectorAll('[data-form-id]').forEach((node) => node.addEventListener('click', () => selectForm(node.dataset.formId)));
}

function renderQuestion(question) {
  const required = question.is_required ? '<span class="danger-text">*</span>' : '';
  const header = `<div><div style="font-weight:800;margin-bottom:6px">${escapeHtml(question.title)} ${required}</div>${question.description ? `<div class="muted">${escapeHtml(question.description)}</div>` : ''}</div>`;
  ...
  if (question.question_type === 'dropdown') {
    return `<div class="question-block">${header}<select class="select js-answer" data-question-id="${question.id}"><option value="">선택하세요</option>${(question.options || []).map((option) => {
      const status = optionClosed(question.id, option);
      const count = getOptionCount(question.id, option.id);
      const cap = option.capacity !== null && option.capacity !== undefined ? ` / ${option.capacity}` : '';
  ...
  if (question.question_type === 'single' || question.question_type === 'multi') {
    return `<div class="question-block">${header}<div class="grid">${(question.options || []).map((option) => {
      ...
      const count = getOptionCount(question.id, option.id);
      const cap = option.capacity !== null && option.capacity !== undefined ? ` / ${option.capacity}` : '';
      return `<label class="option ${status.closed ? 'disabled' : ''}"><input ${status.closed ? 'disabled' : ''} type="${question.question_type === 'single' ? 'radio' : 'checkbox'}" name="q_${question.id}" value="${option.id}" data-question-id="${question.id}" class="js-option"><div><div style="font-weight:800">${escapeHtml(option.label)}</div><div class="muted">신청 ${count}${cap}</div>${option.deadline_at ? `<div class="muted">마감 ${escapeHtml(fmtDate(option.deadline_at))}</div>` : ''}${option.event_start_at ? `<div class="muted">일정 ${escapeHtml(fmtDate(option.event_start_at))}</div>` : ''}${status.closed ? `<div class="pill red" style="margin-top:6px">${escapeHtml(status.reason)}</div>` : ''}</div></label>`;
...
function buildAnswerPayload() {
  ...
  for (const question of selectedBundle.questions || []) {
    if (['short', 'paragraph', 'date', 'time', 'dropdown'].includes(question.question_type)) {
      ...
      answers.push({ questionId: question.id, valueText: question.question_type === 'dropdown' ? '' : value, valueJson: value || null, optionIds: question.question_type === 'dropdown' && value ? [value] : [] });
      continue;
    }
...
async function handleSubmit() {
  try {
    ...
    const result = await submitForm({ formId: selectedBundle.form.id, participantName: payload.participantName, participantPhone: payload.participantPhone, participantEmail: payload.participantEmail, answers: payload.answers });
...
async function selectForm(formId) {
  ...
  renderFormList();
  renderSelectedForm();
  setStatus(statusNode, isDemoMode() ? '데모 폼 준비 완료' : '폼 준비 완료', 'ok');
}
...
async function boot() {
  try {
    ...
    setStatus(statusNode, error.message || '데이터 연결 실패', 'err');
  }
}
