...
import { initStorage, loadData, loadProfile, saveProfile, submitSubmission } from './storage.js';
...
async function submitCurrentForm() {
  ...
  refreshAll();
  if (result.ok) {
    setStatus(result.mode === 'supabase' ? '제출 완료 · Supabase 반영됨' : '제출 완료 · 로컬 저장됨', 'ok');
    return;
  }
  setStatus('제출은 저장됐지만 Supabase 반영은 실패했어. 설정값과 RLS를 확인해줘.', 'err');
}
